    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if AFK is True:
            if mention.sender_id not in USERS:
                await mention.reply(
                    "Hey, Alex here! Sorry, Sammy is AFK due to "
                    + await afk_reason()
...
async def set_afk(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        ...
        if not AFKREASON:
            AFKREASON = 'No reason'
        await e.edit("AFK for now! Cya!")
        if BOTLOG:
            await e.client.send_message(BOTLOG_CHATID, "Sir, You went AFK!")
        await afk(AFKREASON)
